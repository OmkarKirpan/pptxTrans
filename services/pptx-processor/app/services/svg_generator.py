...
async def _get_uno_context_with_retry():
    ...
    # Add LibreOffice UNO paths to sys.path if not already present
    import sys
    uno_paths = [
    ...
    try:
        ...
        for i in range(slide_count):
            try:
                ...
                # Create export filter properties using UNO PropertyValue
                from com.sun.star.beans import PropertyValue
                
                filter_props = (
                    PropertyValue("FilterName", 0, "impress_svg_Export", 0),
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, svg_file in enumerate(svg_files[:slide_count]):  # Limit to expected count
                ...
                # Use shutil.move to handle cross-device link errors
                shutil.move(svg_file, destination_path)
...
async def generate_svgs(
    ...
    """
    Main function to generate SVG files for slides using UNO API only.
    Returns a dictionary mapping slide numbers to SVG file paths.
    """
    logger.info(f"Starting SVG generation for {slide_count} slides using UNO API only", 
                extra={"presentation_path": presentation_path, "output_dir": output_dir, "slide_count": slide_count})
    
    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)
    
    # Use UNO API only as requested
    try:
